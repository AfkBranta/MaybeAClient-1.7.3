...
extends Tab {
    ...
    public void renderIngame() {
        if (ClientInfoHack.instance.status) {
            super.renderIngame();
        }
    }
    ...
    public void render() {
        ...
        String coordY = "";
        String coordZ = "";
        String facing = String.format("Facing: %s", new Object[]{PlayerUtils.getDirection()});
        ...
        if (this.minimized) {
            ...
            if (renderCoords) {
                if (ClientInfoHack.instance.showNetherCoords.value) {
                    ...
                    } else if (ClientInfoHack.instance.isInNether.currentMode.equals("Detect")) {
                    ...
                    if (inNether) {
                        coordX = String.format("X: %.2f %s%.2f", new Object[]{Client.mc.thePlayer.posX, ChatColor.LIGHTGREEN, Client.mc.thePlayer.posX * 8.0});
                        coordY = String.format("Y: %.2f %s%.2f", new Object[]{Client.mc.thePlayer.posY, ChatColor.LIGHTGREEN, Client.mc.thePlayer.posY * 8.0});
                        coordZ = String.format("Z: %.2f %s%.2f", new Object[]{Client.mc.thePlayer.posZ, ChatColor.LIGHTGREEN, Client.mc.thePlayer.posZ * 8.0});
                    } else {
                        coordX = String.format("X: %.2f %s%.2f", new Object[]{Client.mc.thePlayer.posX, ChatColor.LIGHTRED, Client.mc.thePlayer.posX / 8.0});
                        coordY = String.format("Y: %.2f %s%.2f", new Object[]{Client.mc.thePlayer.posY, ChatColor.LIGHTRED, Client.mc.thePlayer.posY / 8.0});
                        coordZ = String.format("Z: %.2f %s%.2f", new Object[]{Client.mc.thePlayer.posZ, ChatColor.LIGHTRED, Client.mc.thePlayer.posZ / 8.0});
                    }
                } else {
                ...
                if ((wid = Client.mc.fontRenderer.getStringWidth(coordY) + 2) > this.width) {
                    this.width = wid;
                }
                if ((wid = Client.mc.fontRenderer.getStringWidth(coordZ) + 2) > this.width) {
                    this.width = wid;
                }
                this.height += 36;
            }
        ...
        if (renderCoords) {
            if (alignRight) {
                rendX = this.xPos + this.width - Client.mc.fontRenderer.getStringWidth(coordX);
            }
            Client.mc.fontRenderer.drawString(coordX, rendX, this.yPos + ++i * 12 + 3 + 2, 0xFFFFFF);
            if (alignRight) {
                rendX = this.xPos + this.width - Client.mc.fontRenderer.getStringWidth(coordY);
            }
            Client.mc.fontRenderer.drawString(coordY, rendX, this.yPos + ++i * 12 + 3 + 2, 0xFFFFFF);
            if (alignRight) {
                rendX = this.xPos + this.width - Client.mc.fontRenderer.getStringWidth(coordZ);
            }
            Client.mc.fontRenderer.drawString(coordZ, rendX, this.yPos + ++i * 12 + 3 + 2, 0xFFFFFF);
        }
        if (renderFacing) {
            if (alignRight) {
                rendX = this.xPos + this.width - Client.mc.fontRenderer.getStringWidth(facing);
            }
            Client.mc.fontRenderer.drawString(facing, rendX, this.yPos + ++i * 12 + 3 + 2, 0xFFFFFF);
        }
        if (renderFPS) {
            if (alignRight) {
                rendX = this.xPos + this.width - Client.mc.fontRenderer.getStringWidth(fps);
            }
            Client.mc.fontRenderer.drawString(fps, rendX, this.yPos + ++i * 12 + 3 + 2, 0xFFFFFF);
        }
        if (renderUsername) {
            if (alignRight) {
                rendX = this.xPos + this.width - Client.mc.fontRenderer.getStringWidth(username);
            }
            Client.mc.fontRenderer.drawString(username, rendX, this.yPos + ++i * 12 + 3 + 2, 0xFFFFFF);
        }
        if (renderSpeed) {
            if (alignRight) {
                rendX = this.xPos + this.width - Client.mc.fontRenderer.getStringWidth(walkingSpeed);
            }
